...
def login(url):
    ...
    if user:
            ...
            response= make_response(render_template("teacher/home.html",
                elapsed_time_seconds= f'{elapsed_time():2.3f}'
                ))
...
def list(course_id):
    ...
    students=[(users.find_one_by_id(ObjectId(s.student)),s.participation   ,str(s.id)) for s in student_ids]
...
def attendance_date(course_id,date):
    ...
    course=courses.find_one_by({'course_id':course_id})
    student_ids=course_student.find_by({'course':str(course.id)})
    students=[(users.find_one_by_id(ObjectId(s.student)),
