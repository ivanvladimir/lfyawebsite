...
def index():
    ...
    if current_user.is_authenticated:
        return render_template(
            "admin/home.html", elapsed_time_seconds=f"{elapsed_time():2.3f}"
        )
    else:
...
def mail_to():
    ...
    if form.validate_on_submit():
        ...
        yag.send(form.to.data, form.subject.data, [markdown.markdown(form.msg.data)])
        return render_template(
            "admin/mail_sent.html",
            form=form,
            elapsed_time_seconds=f"{elapsed_time():2.3f}",
        )

    return render_template(
        "admin/mail_to.html", form=form, elapsed_time_seconds=f"{elapsed_time():2.3f}"
    )
